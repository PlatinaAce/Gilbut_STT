...
import websockets
import threading
from TTS_linux import start_tts_server
...
# 오디오 설정
SAMPLE_RATE = 16000
CHUNK_SIZE = 512  # Silero VAD는 16kHz에서 512 samples 입력 필요
CHUNK_DURATION = CHUNK_SIZE / SAMPLE_RATE

# Linux에서는 오디오 장치 번호가 Windows와 다르므로 처음에는 None으로 두고
# print_audio_devices() 출력 결과를 보고 필요하면 환경변수로 지정하세요.
# 예: INPUT_DEVICE_INDEX=2 python STT_linux.py
_input_device_env = os.getenv("INPUT_DEVICE_INDEX", "").strip()
INPUT_DEVICE_INDEX = int(_input_device_env) if _input_device_env else None
...
# tiny 모델 경로
# Linux에서는 Windows의 F:\ 경로를 사용할 수 없으므로 실제 Linux 경로로 바꾸세요.
# 직접 수정하거나 환경변수로 넣을 수 있습니다.
# 예: export WAKE_MODEL_PATH=/home/user/models/whisper-tiny-gilbut
WAKE_MODEL_PATH = os.getenv(
    "WAKE_MODEL_PATH",
    "/home/user/models/whisper-tiny-gilbut"
)

# turbo 모델 경로
# 예: export COMMAND_MODEL_PATH=/home/user/models/whisper-large-turbo-lora-gilbut
COMMAND_MODEL_PATH = os.getenv(
    "COMMAND_MODEL_PATH",
    "/home/user/models/whisper-large-turbo-lora-gilbut"
)
...
# Spring Boot가 같은 Linux PC에서 돌면 localhost 그대로 사용.
# 다른 PC에서 돌면 해당 IP로 변경.
SPRING_WS_URL = os.getenv("SPRING_WS_URL", "ws://localhost:8080/whisper")
...
def play_ready_sound():
    """
    Linux/Windows 공통 방식의 비프음 재생.
    Windows 전용 winsound 대신 sounddevice로 sine wave를 직접 출력한다.
    """
    print("[INFO] Playing ready sound...")

    duration_sec = BEEP_DURATION / 1000.0
    sample_count = int(SAMPLE_RATE * duration_sec)

    t = np.linspace(0, duration_sec, sample_count, endpoint=False)
    tone = 0.2 * np.sin(2 * np.pi * BEEP_FREQUENCY * t)

    try:
        sd.play(tone.astype(np.float32), SAMPLE_RATE)
        sd.wait()
    except Exception as e:
        # 리눅스에서 출력 장치가 안 잡혀도 STT 전체가 죽지 않게 함
        print(f"[WARN] Failed to play ready sound: {e}")
        # 터미널 bell fallback. 환경에 따라 소리가 안 날 수도 있음.
...
def load_whisper_model(model_path: str, base_model_name: str, use_peft: bool):
    ...
    if not os.path.exists(model_path):
        raise FileNotFoundError(
            ...
            f"Linux 실제 경로로 WAKE_MODEL_PATH / COMMAND_MODEL_PATH를 수정하거나 환경변수로 지정하세요."
        )
